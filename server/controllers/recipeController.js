...
exports.exploreCategoriesById=async(req,res)=>{
    try {
        let catergoryId=req.params.id;
        const limitnumber=20;
        const catergoryById=await Recipe.find({'category':catergoryId}).limit(limitnumber)
        res.render('categories',{title : 'Cooking Blog - CategoriesByCountry',catergoryById});
    } catch (error) {
        res.status(500).send({message:error.message || "Error Occured"});
    }
} 
...
//     try {
//         const res=await Recipe.deleteOne({name : 'New Recipe'});
//     } catch (error) {
//         console.log(error);
//     }
// }
